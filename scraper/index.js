...
class Crawler extends EventEmitter {
    ...
    async init() {
        ...
        const _databaseName = /:\s(.*)$/.exec(this.document.querySelector(".content.container center h5").textContent)[1].trim() + ".sqlite";

        const _token = this.document.querySelector("input[type=hidden][name=jeton]").value;
        this.token = _token;
    ...
    async _parse() {
        this.majors =
            Array.from(this.document.querySelectorAll("select[name=id] option"))
